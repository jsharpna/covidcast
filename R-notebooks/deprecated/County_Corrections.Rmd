...
```{r}
filtered <- counties %>% group_by(geo_value) %>% 
  summarise(
...
county_filtered <- subset(counties, counties$geo_value %in% filtered$geo_value)
```
...
df <- county_filtered %>% 
  ...
  unite(county,c("state","CTYNAME"), sep = ", ") %>%
  mutate(pop = POPESTIMATE2019/100000)
df$county <- as.factor(df$county)

  p <- ggplot(df %>% group_by(county)%>% filter(any(flag == 'TRUE')), aes(x=time_value, y=value/pop))+
        geom_line(aes(color="observed")) +
    geom_line(aes(y=fmean/pop, color="filtered")) + 
    geom_line(aes(y=smean/pop, color="smoothed")) +
